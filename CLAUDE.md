...
| `src/builtins.rs` | 41 Rust-native builtins (arithmetic, comparison, control, dict, string, numeric, parsing, eval control, type introspection, I/O, sequences, sequence constructors, map, filter), `ok_val()` helper for uniform thunk wrapping, `IncludeContext` + thread-local for `$include` with `clear_include_context()`, `standard_builtins()` registry, `create_root_env()`, `create_stdlib_env()` (loads `stdlib/prelude.llt`) |
