...
| `src/builtins.rs` | 28 Rust-native builtins (arithmetic, comparison, control, dict, string, numeric, parsing, eval control, type introspection, I/O), `IncludeContext` + thread-local for `$include` with `clear_include_context()`, `standard_builtins()` registry, `create_root_env()`, `create_stdlib_env()` (loads `stdlib/prelude.llt`) |
| `src/value.rs` | Runtime types: `Value`, `Thunk` (lazy memoization with `Unevaluated`, `PendingBuiltin`, `PendingCall`, `InProgress`, `Materialized`, `Failed` states; `origin` label for stack traces), `Environment` (lexical scope chain), `BuiltinFn` signature (receives call-site `Span`) |
