...
When you recall a finding from a mempalace drawer and need its full details — a specific span propagation path, error formatting pattern, or stack frame construction detail — go back to the source material rather than working from the summary alone. Mempalace entries are compressed pointers; the code in `src/error.rs`, `src/eval.rs`, and `src/builtins.rs` is the ground truth. Use `Read` to re-read the implementation before applying a recalled finding. A half-remembered span propagation path applied confidently is worse than admitting you need to check.
