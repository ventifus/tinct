...
9. **unify_rows field partitioning**: `Row.fields` is `HashMap<String, Type>` (correct — row field order is irrelevant at the type level). The allocation concern is in `unify_rows` itself, not field storage.
