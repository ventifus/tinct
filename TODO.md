...
- [ ] Add corpus tests for TypeAssert Record/proxy error paths — BLOCKED on items 450+451: both error branches have missing `.with_materialization_span()` chains, producing incorrect span output that would make corpus test assertions unstable. Unit tests (23) cover paths in the meantime. (`tests/corpus/eval/errors/`) [Major, test-crafter + span-integrity-checker T4]
