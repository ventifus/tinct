...
## row-unification: Row Variable Unification (Rémy/Kiselyov Model)

Row-polymorphic type inference for dicts: `{ x: Int | ρ }` where `ρ` is a row variable ranging over the unknown remaining fields. Enables functions like `get` and `merge` to preserve known field types. Implementation is ~80% complete (row-unification sprints a through e). The missing piece is row variable binding in the open-record unification case.

**References:** Rémy (1994) — presence/absence flag system. Kiselyov (2013) — level-based generalization. Row-tail level semantics: `RowTail::RowVar(u32)` stores creation-time level; `state.levels` is the authoritative current level map.

### row-unification-f: Partition-Fields-and-Bind
...
- [ ] Implement `partition_fields_and_bind` in `unify_remainders` Case 4 — when `ρ1 ≠ ρ2`, partition `(unique1, shared, unique2)`, create fresh `ρ3` for the common tail, bind `ρ1 := Record(unique2_fields, ρ3)` and `ρ2 := Record(unique1_fields, ρ3)`. Preserves principal types (Harper & Pierce 1991). (`src/types.rs:319-339`) [Major, type-theorist]
- [ ] Add row variable occurs check in `partition_fields_and_bind` — before binding `ρ1 := R`, verify `ρ1 ∉ free_row_vars(R)` to prevent infinite type construction (same pattern as `row_var_occurs_pub` at `src/types.rs:596-608`). (`src/types.rs`) [Major, type-theorist]
- [ ] Add `check_size` call after each `row_map.insert` in `partition_fields_and_bind` — consistent with existing size-capped bindings at `src/types.rs:728-731`. (`src/types.rs`) [Minor, type-theorist]

### row-unification-g: Tests

- [ ] Add corpus test: open-record function `[fn [r@[x: Int]] $r.y]` — should infer `r : { x: Int | ρ }` and access to unknown field `y` produces type error vs. open-record pass-through. (`tests/corpus/eval/typecheck/`) [Major, type-theorist]
- [ ] Add corpus test: `$merge` with open-record operands — `[call $merge [x: 1] $unknown_dict]` should preserve `x: Int` in result type and leave remaining fields open (`ρ`). (`tests/corpus/eval/typecheck/`) [Major, type-theorist]
- [ ] Add corpus test: field access on merged open records — verify that after merging, known fields remain accessible at their inferred types. (`tests/corpus/eval/typecheck/`) [Major, type-theorist]
- [ ] Enable `test_bracket_access_forward_ref_resolves_correctly` in `src/typecheck.rs:3595` — currently `#[ignore]`; un-ignore after `partition_fields_and_bind` is implemented. (`src/typecheck.rs:3595`) [Minor, type-theorist]

### row-unification-h: Doc Section

- [ ] Add §Row-Variable Unification to `doc/07-type-extensions.md` — formal rules for `unify_remainders` Cases 1-4, including the `partition_fields_and_bind` algorithm; reference Rémy (1994) and Harper & Pierce (1991). (`doc/07-type-extensions.md`) [Minor, type-theorist]
- [ ] Document row-tail level semantics in `doc/06-type-inference.md` — `RowTail::RowVar(u32)` stores creation-time level; `state.levels` is authoritative current level; explain why level is stored in the tail type (Kiselyov 2013 §3). (`doc/06-type-inference.md`) [Minor, type-theorist]
