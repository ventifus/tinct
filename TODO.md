...
- [ ] Fix double `format!()` for TypeVar name in infer_dict Pass 1 — `format!("_t{}", state.name_counter)` evaluated twice for the same value (once for TypeVar, once for levels.insert). Bind `let name = format!(...)` once, reuse. (`src/typecheck.rs:276-280`) [Minor, performance-expert C40]
