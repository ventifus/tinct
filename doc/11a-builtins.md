...
| `builtin-typecheck-doc` | `[Fn@Dict [Document TypeContext Dict]]` | Type-check a resolved SurfaceDocument against a TypeContext; requires exactly 3 arguments; updates the TypeContext in-place with new type schemes; third arg is the doc-env Dict — its string-keyed thunks are built into a GroupSpine so `eval_type_stage_expr` resolves type-stage VarRefs from the accumulated environment, and its keys seed `scope_frames` for typeclass instance dispatch; returns `{doc: Document, diagnostics: Dict}` |
...
- `builtin-typecheck-doc`: Requires exactly 3 arguments. Type mismatch if first arg is not Document, second arg is not TypeContext, or third arg is not a Dict (the doc-env Dict passed to `builtin-eval`); type errors returned in `diagnostics` dict rather than raising
