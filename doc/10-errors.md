...
| Cacheability | `ErrorKind::is_cacheable()` method (`error.rs`) |
| Catchability | `ErrorKind::is_catchable()` method (`error.rs`) |
...
| PROP-DEPTH non-caching | `eval.rs` depth check constructs `ErrorKind::DepthExceeded`, defined via `ErrorKind::is_cacheable()` method |
