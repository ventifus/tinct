...
pub enum CoreExpr {
    ...
    Fn {
        /// Clauses of this function. Single-clause for ordinary functions; multi-clause for
        /// instance method dispatchers. Each clause has its own params, optional pattern filter,
        /// guard, body, and capture list. Replaces the former `params`/`body`/`return_ann` fields.
        clauses: Vec<CoreClause>,
        /// Captures for the whole Fn node — used to build closure_env at fn-definition time.
        captures: Arc<Vec<(String, VarAddr)>>,
        /// Some((class_decl_id, method_name)) for instance method functions; None otherwise.
        instance_of: Option<(u64, String)>,
        // True if this lambda was generated by `_` implicit lambda desugaring; false for
        // user-written `[fn [let ...] body]` forms.
        desugared: bool,
        /// Resolved TypeValue.Fn for this function (from type checker). None = gradual.
        resolved_fn_type: Option<Arc<Value>>,
    },
...
/// Replaces the former `CoreMatchArm` struct (T-2126).
///
/// - fn clauses: `params` = declared parameters; `lowered_pattern` = None.
/// - match arm clauses: `params` = binding names as CoreParam; `lowered_pattern` = structural filter.
/// - instance method clauses: `params` = typed CoreParams; `instance_of` set on parent Fn.
///
/// The former `CoreMatchArm.pattern: Arc<SurfaceNode>` and `CoreMatchArm.let_bindings` fields
/// have been removed. Access should use `lowered_pattern` and `params` respectively.
pub struct CoreClause {
    ...
    /// Structural filter expression lowered from the match arm pattern.
    /// `None` for fn clauses and keyed arms (no pattern filter).
    /// `Some(expr)` for case arm clauses — the lowered pattern CoreExpr.
    pub lowered_pattern: Option<Arc<Spanned<CoreExpr>>>,
    pub guard: Option<Arc<Spanned<CoreExpr>>>,
    pub body: Arc<Spanned<CoreExpr>>,
    /// Closure capture list — always present (was `Option` in former `CoreMatchArm`).
...
The parser emits `SurfaceMatchArm` directly with `let_bindings: Some(...)`. The resolver drives case arm scoping from `arm.let_bindings`, calling `enter_param_scope` for the declared names. The lowerer converts `SurfaceMatchArm` to `CoreClause`: binding names become `CoreClause.params`, and `lowered_pattern` is set from the lowered surface pattern.

The evaluator (`eval_materialize.rs`) dispatches on `CoreClause.params`:
- `params.is_empty()` — keyed arm: use `lowered_pattern` to match against the scrutinee
- `params.is_non_empty()` — case arm: inspect `lowered_pattern` head:
  - **Structural** (Var with TypeNode binding, Placeholder) — walk `lowered_pattern` against the scrutinee, bind `params` names to matched fragments via `eval_case_arm_structural_pattern`
  - **Other** — bind all `params` names to the scrutinee, evaluate `lowered_pattern` as a boolean guard predicate
...
`parse()` returns `Result<ParseOutput, TypeDiagnostic>`. `ParseOutput { program: SurfaceProgram, leading_comments: BTreeMap<u64, Vec<String>>, trailing_comments: BTreeMap<u64, String>, blank_before: BTreeMap<u64, bool>, diagnostics: Vec<TypeDiagnostic> }` carries the AST plus comment side-tables for formatter support. Comment maps are keyed by `span_key(start_line, start_col)`. The evaluator and type checker access `.program`; the formatter uses the comment maps directly.
...
|---------------|--------------|-----------|
| `$_ \| f` | WRAP-PIPE: `lhs` is `$_` (implicit arg) | `[fn [let _] [f _]]` — wraps the pipeline in a lambda |
