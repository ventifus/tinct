...
**Pattern syntax.** Dot-access pattern heads (`[Result.Ok v]`, `Color.Red:`) are syntactically assembled by the parser via `flatten_dot_access_to_tag` in `src/ast.rs`. Constructor patterns should use fully qualified dot-access syntax. When a bare uppercase name matches a TyCon name in `tycon_env` with non-empty constructors (e.g., `Shape` when `Shape: [type Circle Square]` is in scope), the type checker emits a T018 warning and auto-qualifies the tag for backwards compatibility (`typecheck_match.rs`). However, bare constructor-name patterns (e.g., `[Ok v]` where `Ok` is a constructor of `Result`, not a type name) do not currently trigger T018 because the `tycon_env` lookup finds no TyCon named `Ok`. Migration to a hard error for all unqualified constructor patterns is tracked in T-1109.
