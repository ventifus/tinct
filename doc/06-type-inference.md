...
Unannotated non-variadic params get a fresh TypeVar at the current level, enabling Hindley-Milner inference for polymorphic functions like `[fn [let x] $x]`. Variadic params without annotations still default to Unknown. This means most functions can omit type annotations and still achieve full polymorphism — the type checker infers parameter types from usage rather than requiring explicit annotations.
