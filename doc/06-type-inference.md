...
Unannotated non-variadic params currently receive `Type::Unknown` (gradual typing). A future refinement will assign fresh TypeVars at the current level to enable full Hindley-Milner inference for polymorphic functions like `[fn [x] x]` — allowing the type checker to infer parameter types from usage rather than requiring explicit annotations. Variadic params without annotations also default to Unknown.
