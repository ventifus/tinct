# Composable Networking (lib-net-v2)

Implemented 2026-05-09 (`connect-v2`, `http-sessions`, `stdlib-protocols` sprints).

## Overview

Tinct's networking stack is built on three orthogonal primitives — Connector, Layer,
and Session — that compose freely. TCP, UDP, Unix sockets, TLS, QUIC, HTTP/1.1,
HTTP/2, HTTP/3, gRPC, WebSocket, and DNS all emerge from these three primitives.

tinct runs at infrastructure boundaries. Infrastructure networking is not "send an
HTTPS request." It includes:
...
A flat API (`connect`, `tls-connect`, `http-connect`) cannot express these. Three
composable primitives express all of them, and any protocol the user invents.
...
HTTP over ICMP simply omits port:
...
`connect` with a `DirCap` opens a Unix socket relative to the cap's directory — consistent
with how `open cap path mode` works for files. The socket's filesystem permissions apply
on top.

**Implementation note:** The implementation uses `openat2` with `RESOLVE_BENEATH` to
resolve the path to a file descriptor, then connects via that fd — not `PathBuf::join` +
raw `UnixStream::connect`, which bypasses the capability sandbox.
...
Any pure-tinct function can be a Layer. A user building an HTTP-over-ICMP tunnel
wraps their `open-icmp-stream` result in a Layer function:
...
A Session is a multiplexed connection: one physical channel that carries multiple
independent logical streams. Sessions are opened from Handles; Handles are opened
from Sessions.

**QUIC Session** — implemented in Rust via `quinn`. QUIC integrates transport, TLS,
and reliable ordered delivery at the UDP level. `quinn` requires exclusive control of
the UDP socket after creation, so `quic-session` is Connector-style rather than a Layer
over an existing UDP Handle:
...
TLS (h2 token). HTTP/3 requires prior discovery: an Alt-Svc header cached from a
previous response, or a DNS HTTPS record (RFC 9460). The first request to a new server
always uses HTTP/1.1 or HTTP/2 — Alt-Svc is a cache mechanism, not first-visit
discovery. Alt-Svc redirects to a different host are validated against the NetCap before
the upgrade connection is opened.
...
Heavy protocol implementations live in `stdlib/protocols/` — included explicitly, not
loaded automatically:

**`protocols/socks5.llt`** (~80–120 lines, pure tinct)

SOCKS5 Layer (RFC 1928 + RFC 1929). Performs the SOCKS5 handshake over an existing
Handle and returns the same Handle type, now routing through the proxy:
...
Supports CONNECT command only (TCP tunneling — RFC 1928 §4): no-auth mode,
username/password auth (RFC 1929), IPv4/IPv6/hostname target address. BIND (§6) and
UDP ASSOCIATE (§7) are out of scope. Requires binary I/O primitives (`read-bytes`,
`byte-at`, `write-bytes`).

**`protocols/dns.llt`** (~200–300 lines, pure tinct over UDP)

DNS wire protocol (RFC 1035). Constructs and parses DNS packets:
...
RFC 1035 §4.1.4 compression pointers require random access into the packet bytes,
loop detection (per RFC 9267), and a pointer-follow depth limit.
...
stdlib/
  ...
  protocols/           # Protocol implementations: each explicit include
    socks5.llt         # SOCKS5 Layer (~80–120 lines pure tinct)
    dns.llt            # DNS wire protocol (~200–300 lines pure tinct)
    grpc.llt           # gRPC-JSON over HTTP/2 (~40 lines pure tinct)
    websocket.llt      # WebSocket upgrade + framing (~80 lines pure tinct)
```

`net.llt` contains: `fetch`, `http-get`, `parse-http-response`, `build-http-request`,
`http-connect-layer`. URI utilities (`uri-params`, `uri-origin`, `uri->string`) are Rust
builtins — always available without include.
...
Handle modification: `raw_tcp: Option<Rc<RefCell<Option<TcpStream>>>>` field added to
`Value::Handle`. This is required for `tls-layer` (STARTTLS use case). `raw_tcp` must
be `Rc<RefCell<Option<TcpStream>>>` (not a plain `Option<TcpStream>`) because
`Value: Clone` is a fundamental contract — `TcpStream` is not `Clone`. When `tls-layer`
calls `.borrow_mut().take()` to extract the stream, the `Option` becomes `None` in all
clones. A second `tls-layer` call on any alias sees `None` and produces a runtime error.

### `stdlib/net.llt`

Updated to use new primitives: `http-get` uses explicit Handle from `connect` rather
than constructing connections internally. `fetch` dispatches across HTTP versions.
`http-connect-layer` replaces the removed `proxy-connect`.

### Cargo Dependencies

- `quinn` — QUIC implementation (brings in `tokio` as a mandatory dependency; runtime strategy documented in TODO `http-sessions` before adding)
- `ipnet` — CIDR range matching in NetCap (new direct dep; already transitive)
- reqwest provides HTTP/2 via `h2` feature and HTTP/3 via `http3` feature
