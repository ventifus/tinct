...
`gensym` names have the form `ℊꜱʏᴍ⧼prefix⧽N` — built from Unicode codepoints (`ℊ` U+210A, `ꜱʏᴍ` small-cap letters, `⧼⧽` U+29FC/29FD angle brackets) that require deliberate IME input. A user cannot accidentally write these in source; collision is practically impossible. The canonical implementation is `make_gensym_name(prefix, id)` in `builtins_meta.rs`. Names are unique but not stable across evaluation orders (lazy forcing may invoke `gensym` in any sequence); this is intentional — `gensym` guarantees uniqueness, not reproducibility.

For macros that follow the gensym convention — using `gensym` for every introduced binding — hygiene is complete without scope sets. In such macros, every name in the body is either (a) pattern-bound from user input via `[let ...]` — inherently in user scope — or (b) gensym'd with `ℊꜱʏᴍ⧼prefix⧽N` naming, which is practically unforgeable. No third category exists where such a macro could accidentally capture user scope. Scope sets (Flatt 2016) address that third category; the gensym convention eliminates it.
