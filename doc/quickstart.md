...
**The parser rule:** if the first element is a bare word (not followed by `:`), it's a call. If the first element is a keyword (`fn`, `match`, `type`, `let`, `class`, `instance`), it's that construct. Otherwise it's a dict.
...
**I/O:** `emit`, `lines`, `open`, `stat`, `exists`, `make-dir`, `rename`, `env`, `list-dir`, `narrow`, `make-string-writer`
