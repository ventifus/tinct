...
**Default output formatter:** The JSON output produced by `tinct eval` is generated by `stdlib/fmt/json.llt` — a pure-tinct JSON serializer that lives in the standard library. This formatter is user-visible: you can inspect it, customize it, or use it directly in your own programs via `[include libdir "fmt/json.llt"]`. If `stdlib/fmt/json.llt` is not found (e.g. when running the binary without the stdlib installed), the CLI falls back to a built-in Rust serializer. The output is indented (2-space pretty-printed) by default.
