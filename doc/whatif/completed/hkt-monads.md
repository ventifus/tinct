...
| `key@"l"` | `key` is a parameter bound to a fresh label TypeVar named by the literal `"l"`; the TypeVar (not `key`) is registered in `kind_env` with `Kind::Label` |

A constrained annotation like `m@Monad` implies `Operator` — no separate kind annotation is needed when the constraint provides the kind. In `key@"l"`, the string literal `"l"` names the label TypeVar (not the parameter) — the resolver creates a fresh TypeVar `l_fresh`, registers `kind_env[l_fresh] = Kind::Label`, and binds `key` to `TypeVar(l_fresh)`. This TypeVar is what appears in `HasField` constraints and `TypeScheme.label_vars`.
...
# key@"k" declares k as a label TypeVar — kind_env["k"] = Kind::Label
[get-field: [fn@[m []] [rec@Record  key@"k"]
  [get key rec]]]

# At call site: k unifies with StringLiteral("host"), field type is inferred
[get-field config "host"]    # → String (if config has host: String)
```

**Label TypeVars** are regular TypeVars registered in `kind_env` with `Kind::Label`. The annotation `key@"k"` (where `"k"` is a string literal as the annotation value) causes the resolver to create a fresh TypeVar for `k` and insert `kind_env[k_fresh] = Kind::Label`. This requires no change to the `TypeVar(String, u32)` Rust representation.
...
- `key@"host"` uses `Label` kind — a label TypeVar names a specific field; it does not participate in row construction or type constructor application. `Seq(TypeVar(l, Label))` is a `KIND-LABEL-ERROR` — `Seq` expects a `*`-kinded argument.
...
- **Kind inference pass** *(hkt-foundation)*: Determine kinds of type expressions before HM inference. Check type constructor variables are used at kind `Operator` in class method signatures. Register label TypeVars with `Kind::Label` when encountered in `key@"k"` annotation positions. Enforce `KIND-LABEL-ERROR`: reject any `Kind::Label` TypeVar in a position expecting `Kind::Type`.
...
### `src/typecheck_annot.rs` — label TypeVar annotations *(hkt-foundation)*

In `resolve_type_name`: when the annotation value is a string literal (not a type name), create a fresh TypeVar, register `kind_env[fresh] = Kind::Label`, and bind the annotation name to it. This is what `key@"k"` desugars to — the string literal `"k"` identifies the label TypeVar name, not the field.

### `stdlib/prelude.llt` — `get`/`get-or`/`get-in` annotations *(hkt-mappable-appendable, after `constraint-annotations`)*
...
- **`constraint-annotations`** (required before prelude migrations) — the `fn@[return: a  constraint: [HasField l d a]]` annotation syntax used in `get`/`get-or`/`get-in` prelude signatures requires the `fn@[...]` named-key metadata dict parsing from this sprint. The type checker and `check_get` changes can land before `constraint-annotations`; only the prelude annotation update is gated on it.
