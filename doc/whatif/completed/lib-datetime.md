...
database, and a `ClockCap` capability that makes `now` injectable for
unit testing?

## Current State

tinct has no date-time types. Timestamps appear in config files (cert
expiry, build metadata, scheduled job windows, log rotation policies,
TOML datetime values) and must currently be stored and manipulated as
raw strings. Arithmetic (`cert expires in N days`), comparison (`is
this window active?`), and timezone conversion are all blocked.

### What's Missing

1. **Timestamp type** — no UTC instant type; cert expiry checks,
   build metadata, and scheduling configs must parse RFC 3339 strings
   manually.
2. **Duration type** — no typed span; timeouts and intervals are bare
   integers with implicit units.
3. **`now`** — no way to get the current time without shelling out.
4. **Timezone conversion** — no way to express a timestamp in a local
   timezone; infrastructure configs that generate cron schedules or
   maintenance windows need this.
5. **TOML datetime** — `stdlib/toml-lite.llt` currently stringifies
   TOML datetime values; they should parse to `Timestamp`.

## Why Date-Time Matters for tinct

**Certificate and secret expiry.** The lib-tls.md `tls-peer-cert`
builtin returns `not-after` as an RFC 3339 string. Without a datetime
type, computing days-until-expiry requires splitting strings. With
`Timestamp` arithmetic: `[/ [timestamp-diff expiry [now clock]] [duration-days 1]]`.

**Scheduled maintenance windows.** Infrastructure configs express
maintenance windows in local time (`"every Sunday 02:00 America/New_York"`).
Generating cron expressions from tinct requires timezone-aware datetime
arithmetic.

**Build metadata.** Reproducible builds require injecting a fixed
timestamp rather than `now`. The `ClockCap` design makes this a first-class
concern: CI passes `--cap-clock-fixed "2026-01-01T00:00:00Z"` and the
build is hermetic.

**Log rotation and retention policies.** `[timestamp-add [now clock] [duration-days -30]]`
computes the cutoff timestamp for a 30-day retention window.
...
The `ClockCap` is consistent with tinct's capability model: `DirCap`
controls filesystem access, `NetCap` controls network access,
`ClockCap` controls time access. Scripts that don't receive a
`ClockCap` cannot observe the current time.
...
Rather than shipping a compiled-in timezone database (adding crate
...
should use `duration-days` with explicit day counts.
...
transitions precisely should use UTC throughout and convert only for
display.
...
`stdlib/toml-lite.llt` currently stringifies TOML datetime values
(`2026-01-01T00:00:00Z` → `"2026-01-01T00:00:00Z"`). Once
`parse-timestamp` exists, `toml-lite.llt` can parse TOML Offset
Date-Time values into `Timestamp` directly.
...
- `pad-left` from `doc/whatif/lib-supplemental.md` §Extended String
  Utilities (used in `format-date`).
- `DirCap` and file reading from `doc/whatif/io.md` (already accepted).
...
- Gallant, A. (2024). "jiff" crate. github.com/BurntSushi/jiff. —
  ...
  support; leading candidate for the implementation crate.
