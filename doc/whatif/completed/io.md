...
For more than two or three sequential writes, pipeline stages are cleaner — the `%` pipeline value carries the handle between stages:
...
```bash
llt --max-memory 64M eval --no-pwd --no-stdin --no-env --timeout 5s script.llt
```
...
[db-pass: [read-file data "secrets/db-password"]]
