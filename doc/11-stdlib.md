...
- **I/O handles** (`open`, `read-all`, `lines`, `write`, `write-atomic`, `flush`, `close` as typeclass methods dispatched to Rust builtins; `has-cap?` and `write-line` in `stdlib/io.llt`) — file/stream I/O with Readable/Writable/Seekable typeclasses; Stdout/Stderr/Stdin nominal types with Writable/Readable instances
