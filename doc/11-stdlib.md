...
| Strings | `builtin-str`, `builtin-split`, `builtin-str-length`, `builtin-str-slice` | `str`, `split`, `replace`, `trim`, `str-to-upper-char`, `str-to-lower-char`, `str-map-chars`, `regex-match?`, `join` | Strings are opaque; all content operations require Rust. `upper`/`lower` are stdlib functions in `strings.llt`. `join` uses an O(n) string builder (dual-dispatch Dict/Seq). `str`, `split`, `str-length`, and `str-slice` have stable aliases for shadowability. |
...
1. `create_root_env()` pre-injects all Rust builtins directly into the bootstrap environment — all 284 builtin functions are registered by name (e.g., `builtin-lt`, `builtin-add`, `eval`, `raise`, `from-json`, `load`, `blake3`, etc.)
...
```text
Bootstrap env: all 284 Rust builtins (builtin-lt, builtin-add, eval, raise, load, blake3, ...)
