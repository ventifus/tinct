...
User-defined function — closure-converted; holds a `Vec<CoreClause>` (one clause per match arm or instance method arm). Ordinary functions have exactly one wildcard clause. Instance methods have `instance_of: Some((class_decl_id, method_name))` to mark them for EffectPerform dispatch.
```rust
Value::Function {
    clauses:     Arc<Vec<CoreClause>>,          // ordered, try first-match
    closure_env: Arc<Vec<Arc<Thunk>>>,          // shared across all clauses
    instance_of: Option<(u64, String)>,         // (class_decl_id, method_name) for instance methods
...
builtin-raise:     [fn@Never [let diag@Diagnostic] ...]
```
...
| `builtin-raise` | `src/lower.rs` (`BUILTIN_RAISE_NAME`), `src/builtins_meta.rs` (`builtin_raise`) | User-level error raising. Accepts a Diagnostic dict with the following fields: `message:` (required String — the human-readable error message) and `notes:` (optional integer-keyed Dict of Strings — additional context notes, e.g. `[0: "matched parameter 0: Integer"  1: "no match for parameter 1: String"]`). `builtin-raise` reads the `"message"` field and the integer-keyed entries of the `"notes"` field (in insertion order), and constructs an `EvalError::UserError` with the message and the notes attached. Additional optional Diagnostic fields (`level`, `kind`, `span`, `secondary-spans`, `help`, `call-stack`, `macro-expand`, `blame`) may be present and are currently ignored by `builtin-raise` but are reserved for future use. Any custom prelude must provide `builtin-raise` as a callable that accepts this Diagnostic dict and raises a runtime error. The canonical name is stored in `src/lower.rs` as `pub(crate) const BUILTIN_RAISE_NAME: &str = "builtin-raise"` (test-only; not used in production lowering). Typeclass method dispatch fallback is now handled by native evaluator dispatch via `VarAddr::EffectPerform` scanning the accumulated group (S-1024). See `doc/dispatch.md` for the unified dispatch design. |
...
| `"trace"` (annotation key, exported as `ANNOTATION_KEY_TRACE` in `src/ast.rs`) | `src/lower.rs` (lowering phase), `src/typecheck_cek.rs` (typecheck phase), `src/eval_materialize.rs` (runtime phase) | When a function's return annotation dict includes a `trace` key, the three pipeline phases emit info-level `Diagnostic` entries. The value must be an `Int` to activate tracing: all three phases guard on `trace_level >= 1` — a value of `0` suppresses tracing in all phases including lowering. Typecheck phase: emits a `trace-fn-type` info diagnostic with the inferred return type. Lowering phase: emits a `trace-lower` info diagnostic with the function's CoreExpr representation. Runtime phase: emits `trace-call` (inputs with parameter names and values) and `trace-return` (function name and return value) info diagnostics. Per-phase routing: (1) Lowering phase (`trace-lower`): emitted via `lower()` → routed to `ctx.runtime_diagnostics` at caller sites in eval, eval_materialize, eval_call, builtins_meta, typecheck_annot — printed to stderr after evaluation completes. (2) Typecheck phase (`trace-fn-type`): emitted to `state.diagnostics` → returned as part of the `builtin-typecheck-doc` diagnostics vec → visible to the caller as type diagnostics, not printed to stderr by the post-eval drain. (3) Runtime phase (`trace-call`, `trace-return`): pushed directly to `thunk_ctx.runtime_diagnostics` during function call evaluation — printed to stderr after evaluation completes. Any function author who writes `@[trace: 1]` (or any `Int >= 1`) on a `fn` return annotation activates all three phases. The canonical constant is `pub const ANNOTATION_KEY_TRACE: &str = "trace"` in `src/ast.rs`. |
