...
| `builtin-raise` | `src/lower.rs` (`BUILTIN_RAISE_NAME`) | Used by MethodDispatcher synthesis as the terminal no-match fallback. The lowerer resolves `builtin-raise` from `scope_frames` and captures it as a ClosureCapture in the synthesized dispatcher `CoreExpr::Fn`. The wildcard arm calls `[builtin-raise "no instance of method '...' for the given argument types"]` when no typed arm matches and no parent dispatcher exists. Any custom prelude must provide `builtin-raise` as a callable that raises a runtime error with its argument as the message string. The canonical name is stored in `src/lower.rs` as `pub(crate) const BUILTIN_RAISE_NAME: &str = "builtin-raise"`. |
