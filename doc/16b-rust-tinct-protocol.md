...
| `builtin-raise` | `src/lower.rs` (`BUILTIN_RAISE_NAME`), `src/builtins_meta.rs` (`builtin_raise`) | Used by MethodDispatcher synthesis as the terminal no-match fallback. The lowerer resolves `builtin-raise` from `scope_frames` and captures it as a ClosureCapture in the synthesized dispatcher `CoreExpr::Fn`. When no typed arm matches and no parent dispatcher exists, the wildcard arm calls `[builtin-raise diagnostic-dict]` where `diagnostic-dict` is a tinct Dict with the following fields: `message:` (required String — the human-readable error message) and `notes:` (optional integer-keyed Dict of Strings — additional context notes, e.g. `[0: "matched parameter 0: Integer"  1: "no match for parameter 1: String"]`). `builtin-raise` reads the `"message"` field and the integer-keyed entries of the `"notes"` field (in insertion order), and constructs an `EvalError::UserError` with the message and the notes attached. Additional optional Diagnostic fields (`level`, `kind`, `span`, `secondary-spans`, `help`, `call-stack`, `macro-expand`, `blame`) may be present and are currently ignored by `builtin-raise` but are reserved for future use. The lowerer synthesizes the `notes` dict dynamically by calling `[builtin-string-concat prefix [builtin-llt-repr [builtin-type-of __dN]]]` for each dispatcher parameter, so each note names which parameter type failed to match. Any custom prelude must provide `builtin-raise` as a callable that accepts this Diagnostic dict and raises a runtime error. The canonical name is stored in `src/lower.rs` as `pub(crate) const BUILTIN_RAISE_NAME: &str = "builtin-raise"`. |
