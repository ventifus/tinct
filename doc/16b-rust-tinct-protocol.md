...
Note: dispatch for `get`, `get?`, `get-in` is currently **name-based string matching** in `src/typecheck_cek.rs` — a known Axiom 1 violation tracked under S-992, which will implement general dispatch and allow these to be all-tinct. `if` must have no special Rust handling — it is a plain tinct function defined in prelude.
