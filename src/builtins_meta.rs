...
/// `builtin-class-dispatch`: Generic typeclass method dispatch at runtime.
///
/// Emitted by the ClassDecl lowering for each class method. Takes the class name,
/// method name, and the method's actual arguments. Extracts dispatch type tags from
/// the arguments' runtime types, constructs the instance binding name via
/// `instance_binding_name`, looks it up in the caller's environment, and calls through
/// to the resolved instance method.
///
/// This is the mechanism that makes class methods available as top-level runtime
/// callables. The class and method names appear as tinct string literals in the
/// emitted code (not as Rust string literals), keeping the Rust runtime genuinely
/// agnostic to which classes and methods exist.
///
/// Args: class_name (String), method_name (String), ...method_args
pub(crate) fn builtin_class_dispatch(
    ctx_arg: BuiltinArgs,
) -> Pin<Box<dyn Future<Output = EvalResult<Arc<Thunk>>>>> {
    Box::pin(async move {
        let BuiltinArgs {
        ...
        reject_named("builtin-class-dispatch", named.as_ref(), call_span.clone())?;
        if args.len() < 3 {
            return Err(EvalError::arity_mismatch(3, args.len(), call_span).into());
        }

        // First two args are the class name and method name (strings).
        let class_val = materialize(&args[0], Some(&call_span), &ctx).await?;
        let method_val = materialize(&args[1], Some(&call_span), &ctx).await?;

        let class_name = class_val.as_str().ok_or_else(|| {
            EvalError::type_mismatch("String", class_val.type_name(), call_span.clone())
        })?;
        let method_name = method_val.as_str().ok_or_else(|| {
            EvalError::type_mismatch("String", method_val.type_name(), call_span.clone())
        })?;

        // Remaining args are the method's actual arguments.
        let method_args: Vec<Arc<Thunk>> = args[2..].to_vec();

        // Extract dispatch type tags from the method args.
        // Materialize each to determine its runtime type, then extract the dispatch prefix.
        let mut type_tags: Vec<String> = Vec::with_capacity(method_args.len());
        for arg_thunk in &method_args {
            let arg_val = materialize(arg_thunk, Some(&call_span), &ctx).await?;
            if let Some(tag) = arg_val.dispatch_type_name() {
                type_tags.push(tag.to_string());
            }
            // If dispatch_type_name returns None, skip this arg (it doesn't contribute
            // to dispatch — like function or capability values).
        }

        // Construct the instance binding name.
        let type_arg_refs: Vec<&str> = type_tags.iter().map(|s| s.as_str()).collect();
        let binding_name =
            crate::type_def::instance_binding_name(class_name, method_name, &type_arg_refs);

        // Look up the instance binding in the caller's environment.
        let instance_thunk = {
            let env_read = caller_env.read().unwrap();
        ...
        let result_thunk = Arc::new(Thunk::new_pending_call(
            instance_fn,
            method_args,
            IndexMap::new(),
            call_span.clone(),
        ...
        // Materialize and return the result.
        let result = materialize(&result_thunk, Some(&call_span), &ctx).await?;
