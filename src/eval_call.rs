...
pub(crate) fn func_label(expr: &Expr) -> Option<Rc<str>> {
    match expr {
        Expr::VarRef(name) => Some(Rc::from(format!("call ${name}").as_str())),
        Expr::DotAccess { .. } => Some(Rc::from("call <dot-access>")),
        Expr::Fn {
            desugared: true, ..
        } => Some(Rc::from("call <auto-generated lambda>")),
        _ => None,
    }
}
...
pub(crate) fn eval_call(
    ...
    let pos_thunks: SmallVec<[Rc<Thunk>; 4]> = args
        .iter()
        .map(|arg| {
...
pub fn invoke_function(ctx: &CallContext) -> EvalResult<Rc<Thunk>> {
    ...
    if let Some(ref label) = ctx.origin {
        thunk = thunk.with_origin(Rc::clone(label));
    }
    Ok(Rc::new(thunk))
}
...
mod tests {
    ...
    fn test_func_label_varref() {
        ...
        assert_eq!(label.as_deref(), Some("call $my_func"));
    }
