...
use crate::ast::{CoreExpr, CoreNamedArg, Expr, NamedArg, Param, Span, Spanned};
...
///
/// Individual arguments are still stored as `Rc<Spanned<Expr>>` inside
/// `Thunk::new_unevaluated` because the per-arg thunks in `eval_call_core` have not
/// yet been migrated to `UnevaluatedState::CoreExpr`. Each arg therefore calls
/// `core_expr_to_expr` internally.
/// TODO(parts-e): migrate eval_call_core args to `UnevaluatedState::CoreExpr` to
/// remove the per-arg `core_expr_to_expr` calls entirely.
pub(crate) async fn eval_call_core(
    ...
    // Wrap positional arguments as Unevaluated thunks (lazy).
    // TODO(parts-e): use Thunk::new_unevaluated_core here to store Arc<Spanned<CoreExpr>> directly.
    let pos_thunks: SmallVec<[Arc<Thunk>; 4]> = args
        .iter()
        .map(|arg| {
            let old_arg = Rc::new(crate::ast_convert::core_expr_to_expr(arg));
            Arc::new(Thunk::new_unevaluated(
                old_arg,
                Arc::clone(env),
                Arc::clone(ctx),
    ...
    // Wrap named arguments as Unevaluated thunks (lazy).
    // TODO(parts-e): use Thunk::new_unevaluated_core here to store Arc<Spanned<CoreExpr>> directly.
    let named_thunks = if named_args.is_empty() {
        IndexMap::new()
    } else {
        let mut m = IndexMap::with_capacity(named_args.len());
        for na in named_args {
            let old_val = Rc::new(crate::ast_convert::core_expr_to_expr(&na.node.value));
            m.insert(
                na.node.name.clone(),
                Arc::new(Thunk::new_unevaluated(
                    old_val,
