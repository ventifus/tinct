...
pub(crate) fn builtin_lte(
    ...
    Box::pin(async move {
        ...
        // a <= b ≡ !(b < a)
        let swapped_args = vec![args[1].clone(), args[0].clone()];
...
pub fn math_builtins() -> Vec<crate::value::BuiltinDef> {
    ...
    vec![
        ...
        builtin!(
            "builtin-to-int",
            crate::builtins::builtin_to_int,
            [Strictness::Seq]
        ),
        builtin!(
            "builtin-to-float",
            crate::builtins::builtin_to_float,
            [Strictness::Seq]
        ),
    ]
}
