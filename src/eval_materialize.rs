...
pub(crate) struct GuardedValidateData {
    pub(crate) thunk: Arc<Thunk>,
    /// TypeValue (Arc<Value>) representing the expected type. Uses the TypeValue representation
    /// (TypeValue.Repr, TypeValue.Record, etc.) — the deleted Type enum is not used.
    pub(crate) expected: Arc<Value>,
    ...
    match cont {
        ...
        Cont::GuardedValidate(data) => {
            ...
            match result {
                Ok(value) => {
                    ...
                    // as_record_typevalue_merged extracts fields from TypeValue.Record or merges
                    // all member fields from a TypeValue.Inter of Records.
                    if let Some(fields) = as_record_typevalue_merged(&expected) {
                        ...
                        // For non-Record types, expected is already an Arc<Value> TypeValue.
                        // Pass directly to value_matches_type without conversion.
        ...
        Cont::TypeAssertCheck(data) => {
            ...
            match result {
                ...
                Ok(value) => {
                    // Record types: use validate_and_wrap_record for shape checking and
                    // specific "record missing field" error messages, mirroring GuardedValidate.
                    // This handles TypeValue.Record and TypeValue.Inter-of-Records.
                    if let Some(fields) = as_record_typevalue_merged(&resolved) {
                        if let Value::Dict { ref entries, .. } = value {
                            ...
                            match validate_and_wrap_record(
                                ...
                                Ok(new_entries) => {
                                    let wrapped = Value::Dict {
                                        entries: new_entries,
                                        type_val: crate::value::unknown_type_val(),
                                    };
                                    return Action::Continue(Ok(wrapped));
                                }
                                Err(err) => {
                            ...
                            // Expected Record but got a non-Dict — fall through to generic
                            // type mismatch handling below. value_matches_type will return false.
                        }
                    }
                        ...
                        if matched {
                                ...
                                &arm.let_bindings
                            {
                                ...
                                let arm_bindings = if is_structural {
                                    ...
                                    // Guard expression: bind ALL [let ...] names to scrutinee,
                                    // then evaluate pattern as a guard expression.
                                    // Build bindings: each name in binding_map gets bound to scrutinee.
                                    let mut guard_bindings: Vec<(u32, Arc<Thunk>)> =
                                        Vec::with_capacity(binding_map.len());
                                    for (_name, &slot) in &binding_map {
                                        let thunk = Arc::new(Thunk::value(
                                            scrutinee_value.clone(),
                                            match_span.clone(),
                                        ));
                                        guard_bindings.push((slot, thunk));
                                    }
                                    guard_bindings
                                };
                                ...
                                // (i.e., one of the binding variables), it matches unconditionally.
                                // Example: [case [let v] v body] — pattern `v` refers to Parameter(0),
                                // which was just bound to the scrutinee. This is an unconditional match,
                                // not a guard that needs truthiness checking.
                                //
                                // WHY: A binding-var pattern unconditionally captures the scrutinee — it is
                                // not a guard predicate. Evaluating it as a guard would require calling the
                                // Matchable instance's `match?` method on the scrutinee itself, which is
                                // circular and semantically incorrect. The pattern is simply an identity
                                // binding, not a guard expression.
                                let pattern_is_binding_var = if !is_structural {
                                    match &lowered_pattern.node {
                                        CoreExpr::Var { addr, .. } => {
                                            matches!(addr, crate::ast::VarAddr::Parameter(_))
                                        }
                                        _ => false,
                                    }
                                } else {
                                ...
                                if !is_structural && !pattern_is_binding_var {
                                    // Evaluate the pattern expression as a guard.
                                    // Push a MatchGuardCheck continuation to handle the result.
                                    // Pattern guards don't have pre-resolved Matchable bindings —
                                    // use None to fall back to dynamic dispatch in MatchGuardCheck.
                                    stack.push(Cont::MatchGuardCheck(Box::new(
                                        MatchGuardCheckData {
                                            arm_idx: i,
                                            ...
                                            guard_matchable_binding: None,
                                        },
                                    )));

                                    return Action::EvalCore {
                                        expr: Arc::clone(lowered_pattern),
                                        frame: arm_frame,
                                        ctx,
                                    };
                                }
...
mod tests {
    ...
    use crate::type_tags::{REPR_INT, REPR_STRING};
    ...
    async fn test_guarded_type_assertion_failure_has_secondary_span() {
        ...
        let expected_type = make_typevalue_repr(REPR_STRING);
    ...
    async fn test_cont_memoize_caches_result() {
        ...
        let peeked = thunk
            .peek_result()
            .expect("thunk must be settled (Materialized or Failed) after forcing")
            .expect("thunk must not have errored after successful materialization");
        assert_eq!(
            peeked,
            &Value::Int {
                n: 42,
    ...
    async fn test_guarded_validate_success_materializes_thunk() {
        ...
        let peeked = guarded
            .peek_result()
            .expect("thunk must be settled after successful validation")
            .expect("thunk must not have errored after successful validation");
        assert_eq!(
            peeked,
            &Value::Int {
                n: 42,
    ...
    async fn test_guarded_validate_failure_with_default_evaluates_default_in_caller_env() {
        ...
        let peeked = guarded
            .peek_result()
            .expect("thunk must be settled after default fallback evaluation")
            .expect("thunk must not have errored after default fallback");
        assert_eq!(
            peeked,
            &Value::Int {
                n: 99,
...
mod cek_lifecycle_tests {
    ...
    async fn test_force_step_materialized() {
        ...
        let peeked = thunk
            .peek_result()
            .expect("Thunk::value must start in Materialized state")
            .expect("Thunk::value must not have errored — invariant violation");
        assert_eq!(
            peeked,
            &Value::Int {
                n: 99,
