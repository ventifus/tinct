...
|-------|------------|---------|
| **kind** | Structured error variant with domain-specific data | `ErrorKind::KeyNotFound { key: "name" }` |
...
The `kind` field is an enum with 26 variants organized by category:

**Access errors (E001-E009):**
- `KeyNotFound { key: String }` — E001
- `UndefinedVariable { name: String }` — E002

**Type errors (E010-E019):**
- `TypeMismatch { context: Option<String>, expected: String, got: String }` — E010
- `TypeAssertFailed { expected: String, got: String }` — E011

**Call errors (E020-E029):**
- `ArityMismatch { expected: ArityBound, got: usize }` — E020
- `NamedArgConflict { param: String }` — E021
- `UnknownNamedArg { name: String }` — E022
- `NamedArgRejected { builtin: String }` — E023

**Value errors (E030-E039):**
- `DuplicateKey { key: String }` — E030
- `DivisionByZero { op: String }` — E031
- `IntegerOverflow { op: String }` — E032
- `FloatNotFinite { builtin: String, value: f64 }` — E033
- `EmptyCollection { op: String }` — E034

**Limit errors (E040-E049):**
- `DepthExceeded { limit: usize }` — E040 (not catchable by `$try`, not cacheable)
- `JsonDepthExceeded { limit: usize }` — E041

**Include errors (E050-E059):**
- `IncludeNotAvailable` — E050
- `IncludeIoError { path: String, detail: String }` — E051
- `IncludeCycle { path: String }` — E052
- `IncludeParseFailed { path: String, detail: String }` — E053
- `IncludeFileTooLarge { path: String, size: u64, limit: u64 }` — E054

**Conversion errors (E060-E069):**
- `ParseConversion { builtin: String, input: String, target: String }` — E060
- `JsonParse { detail: String }` — E061
- `JsonRange` — E062

**Evaluation structure (E070-E079):**
- `CircularDependency { name: String }` — E070

**User-generated (E080-E089):**
- `UserError { message: String }` — E080

**Escape hatch (E090-E099):**
- `Internal { message: String }` — E099
...
The error code `[E0XX]` is a stable identifier derived from the `ErrorKind` variant. Error codes are assigned by category (E001-E009 for access errors, E010-E019 for type errors, etc.). The message is generated by the `Display` implementation of the `ErrorKind` variant.
...
All 26 `ErrorKind` variants map to stable error codes and human-readable messages. The table below shows representative message patterns for common errors:

| ErrorKind Variant | Error Code | Message Pattern | Definition Site |
|-------------------|------------|----------------|-----------------|
| **KeyNotFound** | E001 | `"key not found: {key}"` | Access expression (`$dict.key`, `$dict[key]`) |
