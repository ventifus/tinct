...
versions:
    {{container}} run {{run_flags}} --network=host \
        ...
        {{rust_image}} sh -c "ulimit -s unlimited && cargo run --quiet --bin tinct -- run --no-cwd --cap-fs cwd=.:r --cap-net nc=static.rust-lang.org:443 --cap-net nc=crates.io:443 samples/versions.llt"
